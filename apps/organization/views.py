...
from operation.models import UserFavorite, UserCourse, UserJob
...
class AddUserAskView(View):
    ...
    def post(self, request):
        ...
        if userask_form.is_valid():
            ...
            return HttpResponse("{'status': 'fail', 'msg': '添加时出错'}", content_type='application/json')
...
class AddCourseView(View):
    ...
    def post(self, request):
        ...
        exist_records = UserCourse.objects.filter(user=request.user, course=course)
        if exist_records:
            # 如果记录已经存在，则表示用户取消报名
            exist_records.delete()
            course.students -= 1
            course.save()
        return HttpResponse('{"status": "success", "msg": "取消报名成功"}', content_type='application/json')
...
class AddJobView(View):
    ...
    def post(self, request):
        ...
        exist_records = UserJob.objects.filter(user=request.user, job=job)
        if exist_records:
            # 如果记录已经存在，则表示用户取消报名
            exist_records.delete()
            job.people -= 1
        ...
        #    user_job = UserJob()
        #    if job:
        #        user_job.user = request.user
        #        user_job.job = request.course
        #        user_job.save()
        #        return HttpResponse('{"status": "success", "msg": "已报名"}', content_type='application/json')
        #    else:
        #        return HttpResponse('{"status": "fail", "msg": "报名失败"}', content_type='application/json')
